...
export default class FileUtilities {
  ...
  static copyDirectory(target, destination, replace) {
    ...
    /**
   * Merges into a directory, any subdirectories, and any files from the target to the destination. (Async)
   * @param {String} target - the filepath of the directory to copy
   * @param {String} destination - the filepath to copy it to, including the name of the directory

   */
  static mergeDirectory(target, destination) {
    return new Promise((resolve, reject) => { try {
        ...
        const sourceItems = Files.list(target)
     
        sourceItems.forEach(sourceItem => {
            const destinationItem = destination.resolve(target.relativize(sourceItem))

            if (Files.isDirectory(sourceItem)) {
                this.mergeDirectory(sourceItem, destinationItem).catch(err => reject(err))
            } else {
  ...
  /**
   * Gets data from a url and writes it to the destination. (async)
    ...
    return new Promise((resolve, reject) => {
        new Thread(() => {
            try {
                ...
                IS.close();
                FilePS.close();
                resolve(FileLib.exists(destination))
            } catch(err) {
                reject(err)
            }
        }).start();
